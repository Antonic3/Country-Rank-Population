...
import { fetchCountry } from "../CountrySlice/countrySlice";

export default function Compare() {
  const dispatch = useDispatch();
  const { data, isLoading, errorMessage } = useSelector((state) => state.country);
  const [country1, setCountry1] = useState(null);
  const [country2, setCountry2] = useState(null);
  const navigate = useNavigate();

  useEffect(() => {
    dispatch(fetchCountry('https://restcountries.com/v3.1/all'));
  }, [dispatch]);
              ...
              renderOption={(props, option) => {
                ...
                return (
                  ...
                  >
                    ...
                    {option.idd ? `${option.idd.root}${option.idd.suffixes && option.idd.suffixes[0]}` : ''}
                  </Box>
                );
              }}
              renderInput={(params) => (
